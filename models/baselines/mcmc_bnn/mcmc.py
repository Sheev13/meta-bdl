    ...
    for step in iter_pbar:
        ...
        if metropolis_adjusted:
            log_alpha = model.compute_log_acceptance(
                model, X, Y, *current_stuff, *proposed_stuff, step_size=step_size
            )
            u = torch.rand((1,))
            if u < log_alpha.exp():
                # accept the sample
        ...
        with torch.no_grad():
            metrics["log potential"] = model.U(X, Y, posterior_samples[step]).item()
            metrics["average acceptance"] = acceptance_counter / (step + 1)
        iter_pbar.set_postfix(metrics)

    average_acceptance = acceptance_counter / steps
