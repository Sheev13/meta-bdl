...
def main(
    ...
    bdnp = torch.load(PATH + f'/saved_models/bdnp-gideon', weights_only=False)
    ...
    sigma_y_schedule = [0.45, 0.4, 0.35, 0.3, 0.25, 0.2, 0.15, 0.1, 0.05]
    lr_schedule = torch.linspace(1e-3, 5e-5, len(sigma_y_schedule))

    for i, sigma_y in enumerate(sigma_y_schedule):
        bdnp.likelihood.raw_sigmas.data = (sigma_y*torch.ones((1,))/bdnp.likelihood.upper_bound).logit()
        training_stint = train_meta_model(
            bdnp,
            md,
        ...
        torch.save(bdnp, PATH + f'/saved_models/bdnp-gideon-{sigma_y}')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Gideon retrain")
    ...
    parser.add_argument('--training_steps', type=int, default=4_000, help='The number of training steps')
