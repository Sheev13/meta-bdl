...
def train_meta_model(
    ...
    release_prior_at_step: Optional[int] = None,
    anneal_ctxt_proportion: bool = False,
    ctxt_anneal_start_step: int = 0,
    ...
    # if context proportion plan is provided, ensure it has been provided properly
    if anneal_ctxt_proportion:
        if ctxt_anneal_end_step is None:
            ctxt_anneal_end_step = training_steps
    ...
    for training_step in pbar:
        ...
        if release_prior_at_step is not None:
            if training_step == release_prior_at_step:
                model.trainable_prior(True)
            
        if anneal_ctxt_proportion:
            if training_step > ctxt_anneal_end_step:
                proportion = ctxt_anneal_end_proportion
            elif training_step > ctxt_anneal_start_step:
                a = ctxt_anneal_start_proportion
                b = ctxt_anneal_end_proportion
                c = ctxt_anneal_start_step
                d = ctxt_anneal_end_step
                proportion = a + (training_step - c) * (b - a) / (d - c)
            else:
                proportion = ctxt_anneal_start_proportion
        else:
            proportion = None
